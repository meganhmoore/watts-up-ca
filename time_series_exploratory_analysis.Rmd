...
Given our exploratory analysis (see other script) we first want to apply the boxcox transformation and one order of differencing before we get our baseline tslm models.

```{r}
lambda_1990 = BoxCox.lambda(train_res_1990)
lambda_2005 = BoxCox.lambda(train_res_2005)
lambda_2013 = BoxCox.lambda(train_res_2013)

train_res_1990_boxcox = BoxCox(train_res_1990, lambda_1990)
train_res_2005_boxcox = BoxCox(train_res_2005, lambda_2005)
train_res_2013_boxcox = BoxCox(train_res_2013, lambda_2013)

train_res_1990_boxcox_diff = diff(train_res_1990_boxcox)
train_res_2005_boxcox_diff = diff(train_res_2005_boxcox)
train_res_2013_boxcox_diff = diff(train_res_2013_boxcox)
```

```{r}
plot(train_res_1990_boxcox_diff)
```
...
```{r}
tslm_model_1990 = tslm(train_res_1990_boxcox_diff ~ trend + season)
summary(tslm_model_1990)
accuracy(tslm_model_1990)

tslm_model_2005 = tslm(train_res_2005_boxcox_diff ~ trend + season)
summary(tslm_model_2005)
accuracy(tslm_model_2005)

tslm_model_2013 = tslm(train_res_2013_boxcox_diff ~ trend + season)
