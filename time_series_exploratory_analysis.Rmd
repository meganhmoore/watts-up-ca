...
Do analysis of multiple seasonalities

```{r}
temp_1990 <- periodogram(train_test_res_1990)
temp_2005 <- periodogram(train_test_res_2005)
temp_2013 <- periodogram(train_test_res_2013)
temp_1990$freq
round(temp_1990$spec/1e5, 3)
temp_2005$freq
temp_2013$freq
```

The periodogram shows that there are multiple peaks indicating that there are multiple seasonalities. There seem to be at least 2 seasonal patterns with two large seasonal patterns for all time frames, this seems to relate to the annual (12 month) seasonal pattern and a 6 month seasonal pattern.

```{r}
residential_fourier_1990 <- auto.arima(train_res_1990, xreg=fourier(train_res_1990, 5), seasonal=FALSE)
fourier_forecast <- forecast(residential_fourier_1990, xreg=fourier(train_res_1990, 5, 12)) 
plot(fc)
summary(fit)

residential_fourier_2005 <- auto.arima(train_res_2005, xreg=fourier(train_res_2005, 5), seasonal=FALSE)
residential_fourier_2013 <- auto.arima(train_res_2013, xreg=fourier(train_res_2013, 5), seasonal=FALSE)

```

For the 1990 example at least, the fourier transform produces 20 coefficients.
...
models <- list(
  ...
  list("Residential Fourier - 1990", residential_fourier_1990, train_res_1990, test_res_1990, 12, fourier(train_res_1990, 5, 12)),
  list("Residential Fourier - 2005", residential_fourier_2005, train_res_2005, test_res_2005, 12, fourier(train_res_2005, 5, 12)),
  list("Residential Fourier - 2013", residential_fourier_2013, train_res_2013, test_res_2013, 12, fourier(train_res_2013, 5, 12))
)
```
